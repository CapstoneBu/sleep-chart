...
const Timer = ({ value, onChange, width, buttonHeight, visibleCount = 5 }) => { // 기본값 설정
    ...
    React.useEffect(() => {
        ...
        scrollProps.forEach((props, index) => {
            if (props.ref.current) { // null 검사 추가
                props.ref.current.scrollTo({
                    y: getCenterPositionFromIndex(matchIndex[index]),
                });
            }
        });
    }, [dateString, scrollProps]); // scrollProps 의존성 추가
                ...
                return (
                    <ScrollView
                        ...
                        onScroll={Animated.event(
                            ...
                            { useNativeDriver: false } // 필요 시 true로 설정
                        )}>
                        {renderItems.map((item, index) => {
                            ...
                            return (
                                <Button
                                    key={`${props.key}-${index}`} // key 고유하게 변경
                                    style={{ opacity }}
                                    label={item}
